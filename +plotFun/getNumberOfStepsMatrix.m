function [ out ] = getNumberOfStepsMatrix( sub )
...
out=zeros(size(sub,2),4);
  for i=1:size(sub,2)     
        ...
        if target(2)<0 %For having alpha [-180,180] 
           if target(1)>0
               alpha=180+atand(target(1)/target(2));
           else
               alpha=-180+atand(target(1)/target(2));
           end
        else
            alpha=atand(target(1)/target(2));
        end
        ...
        out(i,1)=alpha;
        out(i,2)=-gama; %I did the otherway positive is cw. negative ccw.
        out(i,3)=dist;
